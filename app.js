...
let editingPostWasPublished = false;
...
// ── Date formatting helper (handles Firestore Timestamp, ISO string, or missing) ──
function fmtDate(ts){
  if(!ts) return '';
  ...
  wrap.innerHTML = posts.map(p=>`
    <div class="bcard" onclick="openBlogPost('${p.id}')">
      ...
      <div class="bcard-body">
        <div class="bcard-date">${fmtDate(p.published_at || p.created_at)}</div>
...
function openBlogPost(id){
  ...
  document.getElementById('readerDate').textContent = fmtDate(post.published_at || post.created_at);
...
async function saveBlogPost(){
  ...
  try {
    ...
    // Stamp published_at the moment a post first goes live (not on every edit/re-save)
    if(published && (!editingPostId || !editingPostWasPublished)){
      payload.published_at = firebase.firestore.FieldValue.serverTimestamp();
    }
...
function editBlogPost(post){
  ...
  editingPostWasPublished = !!post.published;
